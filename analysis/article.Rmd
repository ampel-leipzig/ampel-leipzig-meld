...
After selecting the final model we train and validated it as described in
@harrell1996.
Briefly we develop our model and predict survival at `r ameldcfg$times` days.
...
```{r co2e}
tdp <- c(
    ## brain/batch Intel(R) Xeon(R) CPU E5-2650 v4 @ 2.20GHz
...
elapsed_runtime <- sum(
    subset(tar_meta(), !type %in% c("function", "object", "branch"))$seconds
)

kgco2e <- tdp["snowball"] / 1000 * elapsed_runtime / 3600 * kgco2kwh
kgco2e
...
A single run of the complete calculation pipeline took
`r sprintf("%.1f", elapsed_runtime / 3600)` hours of computation on an
Intel(R) Xeon(R) Gold 6240 CPU (thermal design power: 150 W).
