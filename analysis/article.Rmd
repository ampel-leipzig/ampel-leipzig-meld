...
abstract: >
  **Objectives**:
      The shortage in grafts for liver transplantation requires
  ...
  **Methods**:
      We retrospectively analyzed clinical and laboratory data of 654
      patients who were recruited during the evaluation process for
      liver transplantation at the University Hospital Leipzig.
      After comparing 13 different ML algorithms in a nested cross-validation
      setting and selecting the best performing one we build a new model
      to predict 90-day mortality in patients with end-stage liver disease.

  **Results**:
      Penalized regression algorithms yield the highest prediction performance
      in our ML algorithm benchmark.
      In favour of a simpler model we chose the least absolute shrinkage and
      selection operator (lasso) regression.
      Beside the classical MELD variables INR and bilirubin the
      lasso regression selected cystatin C over creatinine, and
      additionally IL-6, total protein and cholinesterase.
      The new model offers an improved discrimination and calibration than MELD
      and MELD with sodium (MELD-Na) or the MELD-Plus7 risk score.

  **Conclusion**:
      We provide a new ML based model of end-stage liver disease that
      outperforms the classical MELD score for 90-day survival prediction.

  **Keywords**:
      end-stage liver disease; MELD; risk stratification; machine learning;
      survival estimation; IL-6.
...
Additionally women are disadvantage [@sealock2022].
Patients with identical disease state could have very different MELD scores and
thus get different priority on the liver transplantation waiting list.
...
```{r flowchart, results = "asis", eval = knitr::is_html_output(), echo = FALSE, fig.width = 5, fig.height = 5, fig.cap = "Flowchart Inclusion/Exclusion.", dev = c("png", "pdf")}
graph <- "
digraph flowchart {
    graph [splines = false];
...
model-selection-bias to get reliable metrics to choose the best model.
...
@harrell1996.
Briefly we developed our model using a `r ameldcfg$nfold` cross-validation
that was repeated `r ameldcfg$nrepcv` times and finally predicted survival
at `r ameldcfg$times` days.
...
A $p < 0.05$ was considered as statistical significant difference.
...
result in a similar high median performance in the benchmark process.
...
extreme gradient boosting, support vector machines and neural networks yield a
worse performance.
...
The resulting model AMPEL model of end-stage liver disease
(AMELD) uses `r length(cfs)` predictors.
Beside the classical MELD variables INR and (direct) bilirubin the lasso
regression selected cystatin C over creatinine, and
additionally IL-6, total protein and cholinesterase.
...
The 90-day survival estimate of a given patient could be calculated as shown in
\@ref(eq:survcalc).
...
```{r roc, fig.width = 7, fig.height = 7, echo = FALSE, message = FALSE}
...
AMELD offers the best AUROC (`r round(roc["AMELD"], 3)`) across all competitors.
...
```{r bootrcv, echo = FALSE, fig.width = 8, fig.height = 6, fig.cap = paste('Calibration of AMELD in comparison to MELD, MELD-Na and MELD-Plus7.\n The points mark the mean predicted survival of', ameldcfg$m, 'patients per interval against their observed mean survival. The error bars correspond to the 95 % confidence interval of the survival estimate.'), dev = c('png', 'pdf'), echo = FALSE}
par(cex = 0.9)
plot(
    bootrcv,
    what = "calibration",
...
AMELD predicts the medium-risk patients more accurate.
Because of the low number of very ill patients in our cohort the prediction
performance in high-risk patients is poor but comparable to the other models.
This would require a recalibration in a larger validation cohort.
...
random forest model the sodium gains any important weight
(`r bsranktxt` and `r rfranktxt`, see \@ref(fig:vimpbs), \@ref(fig:vimprf)).
In addition sodium levels can be affected by diuretic therapy and already
@kartoun2017 have shown that the discrimination performance of MELD-Na is
insignificantly different from the classical MELD.
Consequentially it is not a component of our model.

It has shown that women receive in median lower scores in MELD-Na and therefore
...
across different clinical laboratories and methods.
It also reduce the influence of age- and sex-related differences,
...
Half of them, INR, (direct) bilirubin, and cystatin C are identical
or similar to the labortory measurements utilized in the classical MELD.
...
This is inline with recent studies that show a better prediction of mortality
for cystatin C in unselected and critical ill patients
[@helmerssonkarlqvist2016; @helmerssonkarlqvist2021]

Furthermore AMELD extends MELD by additional synthesis parameters
...
finding clinical meaningful patterns and guide further research.
The systemic inflammatory response syndrome is an
independent risk factor for poor outcome in patients with cirrhosis
[@thabut2007].
However neither MELD nor MELD-Na taking inflammation into account.
In contrast IL-6 is the most important variable in our model. Even if IL-6 is
excluded the algorithms select CRP as one of the most important variables which
highlights the important role of inflammation in the prognosis of patients
with end-stage liver disease.
We and other researchers have already shown that adding inflammatory markers,
namely IL-6 or CRP, to the classical MELD score increase the accuracy of the
90-day mortality prediction [@remmler2018; @cervoni2016].

Beside laboratory measurements we investigated
`r (ncol(raw_data) - length(cn_labs) - 2)`
clinical data but none of them were chosen in our model selection process.
@kartoun2017 also used the lasso approach and neither select any clinical
data. Especially in retrospective analysis the clinical data are often not
detailed enough to keep valuable information.

In our benchmark of `r n_lrns` different machine learning algorithms we have
seen that the more complex algorithm like boosting, support vector machines
and neural networks perform worse than simpler ones.
This may due additive effects that favor regression models or due to our
...
<!--
As we shown above machine learning can aid in finding clinical meaningful
patterns.
-->
...
Principally it is a single center, retrospective analysis and without an
...
We excluded all patients who had already a liver transplantation
...
our cohort was healthier than in previous studies that reported
...
`r paste(round(range(tbl_observed_vs_expected_mortality$StandardizedMortalityRatio[-1]), 1), collapse = " to ")`
times.
Unfortunately our sample size is low in the high risk patient category
which yield a poor but better than MELD based prediction performance of our
...
(direct) bilirubin, cystatin C (instead of creatinine) by
synthesis parameters total protein and cholinesterase and
most importantly the inflammatory marker IL-6.
Using these `r length(cfs)` laboratory measurments the AMELD
...
Additionally all data are available in the `ameld` R package [@R-ameld] at
...
HPC cluster of the University Greifswald and
Bernd Klaus for helpful discussions about survival analysis and proof reading the manuscript.
...
The study was conceptualised by SG and TK.
The data was acquired by TK.
The data was analysed and processed by SG.
Visualisation were produced by SG.
Original drafts of the manuscript were written by SG and TK.
Drafts were edited and reviewed by SG, TB, DS, BI, and TK.
...
regulations, institutional policies and is in accordance with the tenets of
the Helsinki Declaration (as revised in 2013), and has been
