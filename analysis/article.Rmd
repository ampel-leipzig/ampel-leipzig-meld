...
subtitle: >
    Comparision against MELD, MELD-Na, MELD 3.0, MELD-Plus7.
abstract: >
  **Objectives**:
  ...
  **Results**:
      ...
      and MELD with sodium (MELD-Na), MELD 3.0, or the MELD-Plus7 risk score.

  **Conclusion**:
      We provide a new machine-learning-based model of end-stage liver disease
      that incorporates synthesis and inflammatory markers and
      may improve the classical MELD score for 90-day survival prediction.
...
Recently MELD 3.0 was introduced adding a factor for female sex, albumin and
interactions between bilirubin and sodium and between albumin and creatinine
[@kim2021].
...
In addition women are disadvantaged in MELD-Na [@sealock2022].
...
As shown by the MELD-Plus risk and MELD 3.0 scores, better predictive scores
often need more variables and are more complicated.
To reduce the risk of overlooking or
...
stratification for liver transplantation of the classical
MELD, MELD-Na, MELD 3.0, and MELD-Plus scores and that might be implemented as
part of the AMPEL clinical decision support system.
...
MELD 3.0 was calculated as described in kim2021:

$$
\begin{aligned}
MELD3 = 1.33 (if female) &+ \\
        4.56 &* \ln(bilirubin [mg/dL]) +  \\
        0.82 &* (137 - Na [mmol/L]) - 0.24 * (137 - Na [mmol/L]) * \ln(bilirubin [mg/dL]) + \\
        9.09 &* \ln(INR) + \\
        11.14 &* \ln(creatinine [mg/dL]) + \\
        1.85 &* (3.5 - albumin [g/dL]) - 1.83 * (3.5 - albumin [g/dL]) * \ln(creatinine [mg/dL]) + 6
\end{aligned}
$$
...
L = 8.53499496 &+ \\
    ...
    6.34990436 &* \log_{10}(1 + albumin [g/dL]) + \\
...
The final model was compared against the MELD, MELD-Na, MELD 3.0 and MELD-Plus7
scores using the area under the time-dependent receiver operating
characteristic curve (AUROC) based on the
nonparametric inverse probability of censoring weighting
...
Further we determined the variable importance by counting how often the
variable was chosen during the feature selection in the bootstrap process
described above and by the logrank value returned by one random forest
implementation [@R-ranger].
...
(`r .tbl1("CLI")`).
At evaluation time, the median (IQR) MELD, MELD-Na and MELD 3.0 scores were
`r .tbl1("ScoreMeldUnos")`,
`r .tbl1("ScoreMeldNaUnos")`, and
`r .tbl1("ScoreMeld3")`, respectively.
...
We divided our cohort into
`r nlevels(labelled_meld_data$MeldCategory)` MELD-Na-score risk
...
times higher than the predicted one [@vanderwerken2021].
Only in the group with the lowest MELD-Na scores was the mortality much lower.
...
```{r roc, fig.width = 7, fig.height = 7, echo = FALSE, message = FALSE, fig.cap = "Receiver operating characteristic (ROC) curve. Area under the time-dependent ROC curve (AUC) based on the nonparametric inverse probability of censoring weighting estimate (IPCW) for AMELD, MELD, MELD-Na, MELD 3.0, MELD-Plus7, as described in [@blanche2013]."}
col <- viridisLite::viridis(6)
m <- list(
    AMELD = timeROC_RCV,
    MELD = timeROC_MELD,
...
pAUC <- function(auc, ci) {
    sprintf("AUROC %0.3f %s", auc, ci)
}
```
...
```{r selvar, echo = FALSE}
bsrank <- rank(-ameld:::.selvars(bootrcv$models, bootrcv$s), ties = "first")

bsrankSodium <- as.vector(bsrank["NA_S"])

bsranktxtSodium <-
    if (!is.na(bsrankSodium)) {
        paste0(
            "in bootstrap selection rank ", bsrankSodium,
            " of ", ncol(amelddata$x)
        )
    } else {
...
rfrank <- rank(-rngr$variable.importance, ties = "first")
rfrankSodium <- as.vector(rfrank["NA_S"])
rfranktxtSodium <- paste0(
    "in a random forest variable importance rank ", rfrankSodium, " of ",
    ncol(amelddata$x)
)
```

The AMELD selected features
`r enum(gsub("^([A-Z])([a-z])", "\\L\\1\\2", cfnms, perl = TRUE)) `
are the `r max(bsrank[names(cfnms)])` highest ranked features in the bootstrap
process. In the random forest logrank they are amoung the
`r max(rfrank[names(cfnms)])` highest ranked features out of `r length(rfrank)`
features.
Sodium and albumin introduced by MELD-Na and MELD 3.0 are on position
`r paste(bsrank[c("NA_S", "ALB_S")], collapse = " and ")` for the bootstrap and
`r paste(rfrank[c("NA_S", "ALB_S")], collapse = " and ")` for the random forest
variable importance ranking, respectively
(see \@ref(fig:vimpbs), \@ref(fig:vimprf) for the complete rankings).
...
90-day survival probability. Our new model, AMELD,
incorporates synthesis and inflammatory markers and
may improve the prediction over previous published
MELD scores.
...
(`r bsranktxtSodium` and `r rfranktxtSodium`, see \@ref(fig:vimpbs), \@ref(fig:vimprf)).
...
The albumin included in MELD 3.0 is part of the total protein that has
throughout higher rankings in both variable importance measurements
(`r bsrank["ALB_S"]` vs. `r bsrank["PROT_S"]` in the bootstrap selection
and `r rfrank["ALB_S"]` vs. `r rfrank["PROT_S"]` in the random forest variable
importance, see \@ref(fig:vimpbs), \@ref(fig:vimprf)).
Using total protein instead of albumin as synthesis marker
offers more information about additional globulins and especially
...
[@sealock2022].
MELD 3.0 tries to circumvent this by adding 1.33 to the score for femal sex.
...
[@thabut2007].
However, neither MELD, MELD-Na, nor MELD 3.0 took inflammation into account.
...
With a median MELD-Na score of `r .tbl1("ScoreMeldNaUnos", pattern = "{median}")`,
our cohort was healthier than those in previous studies that reported
a score of 14-18.5 [@kim2008; @kartoun2017; @vanderwerken2021].
...
Drafts were edited and reviewed by SG, TB, AH, BI, and TK.
