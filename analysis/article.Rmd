...
If more than 10 % of laboratory measurements are missing the variable were
excluded from further analysis.
The remainder of missing laboratory measurements were replaced with
a *zlog* value of zero.
...
(IPCW) [@blanche2013, @R-timeROC]. Testing was applied as described in @blanche2013. A $p < 0.5$ was considered as statistical significant difference.
...
invisible(plot_surv_roc(m, timepoint = 90, col = col[-2]))
```
...
knitr::kable(
    tp,
    caption = paste0(
        "Test comparison between AUC for MELD-Na and RCV model ",
...
```{r bootrcv, fig.width = 7, fig.height = 7, echo = FALSE}
tar_load(ameldcfg)
tar_load(amelddata)
tar_load(zlog_data)
par(cex = 0.9)
...
legend("topleft", col = col[3:5], legend = names(ps), pch = 19, bty = "n")
```
...
(in bootstrap selection rank
`r as.vector(rank(-ameld:::.selvars(bootrcv$models, bootrcv$s))["NA_S"])`
of
`r ncol(amelddata$x)` and in random forest variable importance
`r as.vector(rank(-rngr$variable.importance)["NA_S"])` of
`r ncol(amelddata$x)`,
see \@ref(fig:vimpbs), \@ref(fig:vimprf)).
...
- comparison of different predictors of MELD/MELD-Plus7 (+MELD-Plus9) vs our features

Results from machine learning can aid in finding clinical meaningful patterns
and guide further research.
For example, the systemic inflammatory response syndrome is an
...
In contrast IL6 is the most important variable in our model. Even if IL6 is
...
namely IL6 or CRP, to the classical MELD score increase the accuracy of the
