...
.bp <- function(x, formula, col = cividis(ncol(x)), main = NULL,
    ...
    beeswarm::beeswarm(formula, data = x, col = col, add = TRUE, pch = 19L)
}
```
...
```{r histograms_rvsz_sex, results = "asis"}
...
for (v in labv) {
    ...
    .bp(log_data, fm, main = "log()")
    .bp(zlog_data, fm, main = "zlog()")
}
par(old.par)
```
...
```{r histograms_rvsz_status, results = "asis"}
old.par <- par(no.readonly = TRUE)
par(mfrow = c(1L, 2L))
for (v in labv) {
    cat('\n\n### ', v, '\n\n')
...
```{r histograms_rvsz_sex_status, results = "asis"}
old.par <- par(no.readonly = TRUE)
par(mfrow = c(1L, 2L))
col <- cividis(4L)
for (v in labv) {
